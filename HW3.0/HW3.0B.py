...
def NN_eval(x,submatrices):
    out = x
    for i in range(0,int(len(submatrices)/2)):
        idx = int(i*2)
        out = submatrices[idx+1].T+np.matmul(out,submatrices[idx].T)
        if (i<int(len(submatrices)/2)-1):
            out = activation_f(out)
    return out
...
def loss(p, index_2_use):
    ...
    if (GAMMA_L1 != 0.0):
        training_loss = training_loss + GAMMA_L1 * np.sum(np.absolute(p))
    if (GAMMA_L2 != 0.0):
        training_loss = training_loss + GAMMA_L2 * np.sum(p ** 2.0)
