...
column_names = ['MPG','Cylinders','Diisplacement','Horsepower','Weight','Acceleration','Model Year','Origin']
df = pd.read_csv(url,names = column_names,na_values='?',comment = '\t',sep = ' ',skipinitialspace=True)

pd.set_option('display.max_columns', None)
SBV.get_pd_info(df);

## the info above shows horsepower contains 6 NA values
...
X_KEYS = ['Cylinders','Diisplacement', 'Horsepower', 'Weight',
       'Acceleration'];
Y_KEYS = ['MPG']
NFIT = len(X_KEYS)+1
...
Y = []
for key in df.keys():
    if (key in X_KEYS): X.append(df[key])
    if (key in Y_KEYS): Y.append(df[key])
...
def minimizer(loss, algo='GD', LR=0.01, method='batch'):
    ...
    if(algo == 'GD'):
        ...
        tmax = 2000  # MAX NUMBER OF ITERATION
...
def plot_1(xla='x', yla='y'):
    ...
    for i in range(0,X.shape[1]):
        ...
        col = df.columns[i+1]
        XT = df[col]
        ax.plot(XT[train_idx], Y[train_idx], 'o', label='Training')
        ax.plot(XT[val_idx], Y[val_idx], 'x', label='Validation')
        ax.plot(XT[test_idx], Y[test_idx], '*', label='Test')
        ax.plot(XT[train_idx], YPRED_T, '.', label='Model')
        plt.xlabel(col, fontsize=18);
        plt.ylabel(df.columns[0], fontsize=18);
        plt.legend()
        plt.show()
